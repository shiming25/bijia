...
class Entities {
	...
	private static final String[][] HTML40_ARRAY = { { "fnof", "402" },
			...
			{ "hearts", "9829" }, { "diams", "9830" }, { "OElig", "338" },
			...
			if (c == '&') {
				...
				if (semiColonIdx == -1) {
					...
					if ((amphersandIdx != -1) && (amphersandIdx < semiColonIdx)) {
						...
						String entityContent = str.substring(nextIdx,
								semiColonIdx);
						int entityValue = -1;
	...
	static class BinaryEntityMap extends Entities.ArrayEntityMap {
		...
		public void add(String name, int value) {
			...
			insertAt = -(insertAt + 1);
			System.arraycopy(this.values, insertAt, this.values, insertAt + 1,
					this.size - insertAt);
			this.values[insertAt] = value;
			System.arraycopy(this.names, insertAt, this.names, insertAt + 1,
					this.size - insertAt);
	...
	private static abstract interface EntityMap {
	...
	static class PrimitiveEntityMap implements Entities.EntityMap {
		...
		private final PrimitiveIntHashMap mapValueToName = new PrimitiveIntHashMap();
